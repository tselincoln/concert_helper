...
class SiteProfiler:
    ...
    async def profile_site(self, target_url: str, sandbox_url: str):
        ...
        logger.info(f"Starting STEALTH recon for domain: {domain} using sandbox: {sandbox_url}")

        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True)
            
            context = await browser.new_context(**StealthManager.get_human_context_args())
            page = await context.new_page()

            await StealthManager.apply_stealth(page)

            try:
                await page.goto(sandbox_url, wait_until="domcontentloaded", timeout=60000)
                
                await StealthManager.human_delay()
                await StealthManager.jitter_mouse(page)
                
                logger.debug("Sandbox navigation and humanity checks complete. Scanning...")
